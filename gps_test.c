...
int main(int argc, char **argv){
	...
	// Set baudrate
	nmea_build_cmd(buf, CMD_SET_BAUDRATE, "9600", (char *) NULL);
	if (gps_write(buf) == -1) printf(ERR_WRITING, buf);
	else{
		printf(WRITTEN_CMD_LOG_PATTERN, buf);
		printf("%s\n", "Waiting for GPS baud rate to be updated");
		sleep(5);
		if (gps_change_baudrate("9600") == -1) printf("%s\n", "Error while changing baudrate");
		else printf("%s\n", "Successfully changed baudrate");
	}
